...
namespace ZSharpVault15lib
{
    public class FileHelper
    {
        ...
        public static string downloadVaultFile(VDF.Vault.Currency.Connections.Connection connection, string path, File selectedFile)
        {
            try
            {
                if (selectedFile != null)
        ...
        public static string getFileIDsinCSV2(VDF.Vault.Currency.Connections.Connection connection, List<File> filesColl)
        {
            resultVal = null;
            try
            {
                ...
                if (filesColl.Count > 0)
                {
                    foreach (File selectedFile in filesColl)
                    {
                        // Look of the File object. 
                        if (selectedFile != null)
                        {
        ...
        private static Dictionary<long, string> resultDict = new Dictionary<long, string>();

        public static Dictionary<long, string> getFilePathList(VDF.Vault.Currency.Connections.Connection connection, List<ISelection> filesColl)
        {
            try
            {
                resultDict.Clear();
                ...
                if (filesColl.Count > 0)
                {
                    foreach (var file in filesColl)
                    {
                        ...
                        {
                            //resultVal += selectedFile.Id + "-";
                            resultDict.Add(selectedFile.Id, selectedFile.Name);
                        }
                        //MessageBox.Show(String.Format("Hello World! The file {0} size is: {1} bytes", selectedFile.Name, selectedFile.FileSize));
                    }
